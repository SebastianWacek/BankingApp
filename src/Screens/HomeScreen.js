...
import { Menu, MenuTrigger, MenuOptions, MenuProvider, MenuOption } from 'react-native-popup-menu';
...
export default HomeScreen = () => {
  ...
  const renderOperations = ({ item }) => (
    <Box style={styles.operations}>
      ...
      <Text
        style={[
          styles.price,
          item.price < 0 ? { color: '#EE5656' } : { color: '#59F82A' }
        ]}
      >
        {item.price} PLN
      </Text>
  ...
  return (
    <Box flex="1" safeArea w="100%" h="100%" bgColor={"#534582"} style={styles.container}>
      <Box style={styles.header}>
        <Image source={require("../Images/pp.png")} style={{ width: 60, height: 60, borderRadius: 10, borderWidth: 1, marginRight: 8, marginLeft: 8 }} />
